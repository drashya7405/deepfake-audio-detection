...
from tensorflow.keras.layers import Input, Dense, LSTM, GRU, Bidirectional, Conv1D, MaxPooling1D, Dropout, BatchNormalization
...
# New shape: (num_samples, num_features, 1). This allows Conv1D to treat the features as a sequence.
...
print("\n\n--- Building Model 1: Bidirectional LSTM (BiLSTM) ---")
...
# Call create_optimizer() to get a NEW optimizer instance
bilstm_model.compile(optimizer=create_optimizer(), loss='binary_crossentropy', metrics=['accuracy'])
bilstm_model.summary()
print("\n--- Training BiLSTM Model ---")
bilstm_history = bilstm_model.fit(X_train, y_train, epochs=5, batch_size=32, validation_data=(X_val, y_val),
                                  callbacks=[early_stopping, lr_scheduler], verbose=1)
plot_history(bilstm_history, 'BiLSTM')
results_list.append(evaluate_model(bilstm_model, X_test, y_test, 'BiLSTM'))

# --- Model 2: Gated Recurrent Unit (GRU) ---
print("\n\n--- Building Model 2: Gated Recurrent Unit (GRU) ---")
gru_model = Sequential([
    Input(shape=(X_train.shape[1], X_train.shape[2])),
    # This line is corrected to ensure it returns a 3D tensor for the next GRU layer
...
# Call create_optimizer() again to get another NEW optimizer instance
gru_model.compile(optimizer=create_optimizer(), loss='binary_crossentropy', metrics=['accuracy'])
gru_model.summary()
print("\n--- Training GRU Model ---")
gru_history = gru_model.fit(X_train, y_train, epochs=5, batch_size=32, validation_data=(X_val, y_val),
                            callbacks=[early_stopping, lr_scheduler], verbose=1)
plot_history(gru_history, 'GRU')
results_list.append(evaluate_model(gru_model, X_test, y_test, 'GRU'))

# --- Model 3: Enhanced Hybrid CNN + RNN Model ---
print("\n\n--- Building Model 3: Enhanced Hybrid CNN-RNN ---")
...
# Call create_optimizer() a third time for the final NEW optimizer instance
...
try:
    ...
    save_path = os.path.join(BEST_MODELS_DIR, '/content/drive/MyDrive/SoftComputing/BestModels/drashya_best_deepfake_audio_model.h5')
